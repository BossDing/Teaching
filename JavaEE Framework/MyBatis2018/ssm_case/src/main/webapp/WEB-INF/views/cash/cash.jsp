...
<body>
	...
	<div id="tb" style="height: auto">
		...
		<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-edit" onclick="openEditWin('editWin', 'list', 'editForm', 'cash')">修改</a>
		<div>
			<form id="searchForm">
				<input class="easyui-numberbox easyui-validatebox" data-options="prompt:'请输入最大金额',
						required:false,
						novalidate:true,
						precision:2" name="cashQuery.money"/>
				<input class="easyui-textbox easyui-validatebox" data-options="prompt:'请输入客户名称',
						required:false,
						novalidate:true" name="cashQuery.cashCustomer"/>
				<input class="easyui-datetimebox" name="cashQuery.cashTime"
					   data-options="prompt:'请选择最大支付时间',required:false,novalidate:true,showSeconds:true"/>
				<input class="easyui-datetimebox" name="cashQueyr.createTime"
			...
			<table>
				<tr>
					...
					<td>
						...
						novalidate:true" name="cash.cashCustomer"/>
					</td>
				</tr>
				<tr>
					<td>收入或支出</td>
				...
				<tr>
					...
					<td>
						<input class="easyui-combobox easyui-validatebox cashType" data-options="prompt:'请选择收支子分类',
						required:true,
						novalidate:true" name="cash.cashType"/>
					</td>
				</tr>
				<tr>
					<td>金额</td>
				...
				<tr>
					...
					<td>
						<input class="easyui-combobox easyui-validatebox payType" data-options="prompt:'请选择支付方式',
						required:true,
						novalidate:true" name="cash.payType"/>
					</td>
				</tr>
				<tr>
					<td>收支时间</td>
					<td>
					<input class="easyui-datetimebox" name="cash.cashTime"
...
<script>
	$(function() {
	    ...
	    $(".cashTypeParent").combobox(
            {
                url:contextPath + '/cash_type/get?pid=0',
                valueField:'id',
                textField:'text',
				onSelect:function(record) {
                    $(".cashType").combobox(
                        {
                            url: contextPath + '/cash_type/get?pid=' + record.id,
                            valueField: 'id',
                            textField: 'text'
                        }
					);
